...
> 🪦 **Documento superado.** La guía editorial vigente del paquete es
> [`../../prompts/guia-de-estilo-y-convenciones.md`](../../prompts/guia-de-estilo-y-convenciones.md),
> que cubre los dos cursos y añade la sección de tuteo (§4.6–§4.7) y la
> nomenclatura de archivos vigente (§13.2). Este archivo se conserva como
> referencia histórica del Track B; ante cualquier discrepancia, **gana la
> guía del paquete**.
...
backend real que honra el mismo contrato (`../00-audit-contrato.md`). Todo lo que
...
- **Eventos de socket:** `recurso:acción` en inglés — `ticket:created`, `ticket:updated`, `ticket:deleted` (los tres del contrato; cualquier otro se declara como extensión, ver `../00-audit-contrato.md`).
...
- **No contradecir fases anteriores** (ni dentro de un track ni respecto del otro). Un fragmento de la Fase 6 de Mongo no puede devolver una forma de `ticket` distinta de la que el frontend de Track A espera; el **contrato** (`../00-audit-contrato.md`) manda.
- **No reescribir decisiones aprobadas** sin señalar explícitamente la incompatibilidad y por qué.
- **Fuentes de verdad, en orden:** (1) instrucciones del proyecto, (2) `../00-audit-contrato.md` (el contrato que une los dos tracks), (3) `plan-formacion-nosql-mongodb.md` y el plan de Track A, (4) entregables aprobados de fases anteriores, (5) decisiones explícitas del chat actual.
...
- [ ] No contradice ninguna fase anterior **ni el otro track** (ni en pedagogía ni en nombres de código); respeta `../00-audit-contrato.md`.
