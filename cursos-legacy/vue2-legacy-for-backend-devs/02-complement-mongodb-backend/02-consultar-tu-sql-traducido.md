...
| `GET /tickets?q=impresora` | `find({ $or: [ { title: /impresora/i }, { description: /impresora/i } ] })` — alcance de `q` firmado en `00-audit-contrato.md` |
...
> [`forense-fase-02.md`](forense-fase-02.md).
...
>
> ```bash
> git tag -a fase-02-consultar-tu-sql-traducido -m "F2 cerrada: <el checklist, en una línea por ítem>"
> ```
>
...
> `ej/f02/17`, y un incidente resuelto en el par `inc/f02/<slug>-roto` /
...
|---|---|---|---|
| 03 | "El reporte de sin asignar da dos números distintos" | Consultas | 🟡 |
