...
reales los archivos. Pero `SECURITY-NOTES.md` — el cuaderno que vienes
llenando desde la Fase 2 del Curso 01, entrada tras entrada de "esto lo
...
9. Sube un `.html` con `<script>alert(1)</script>` como adjunto y ábrelo desde el navegador vía tu `GET /attachments/:id/download`. ¿Se ejecutó? Verifica que `Content-Disposition: attachment` está y funciona.
