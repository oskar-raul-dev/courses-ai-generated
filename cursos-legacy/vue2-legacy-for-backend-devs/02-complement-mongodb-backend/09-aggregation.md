...
pactada en `00-audit-contrato.md`).

> 📝 **Nota de coherencia:** `00-audit-contrato.md` (tabla de extensiones) fichó
...
- la lógica de `GET /stats` completa: por estado, por prioridad, por agente
  ...
  Track A calcula hoy en cliente, Curso 01 · F7), tiempos de resolución desde
...
y el porcentaje de resueltos. El backend lo hará en UNA pasada con `$facet`
(sub-pipelines paralelos sobre el mismo flujo de entrada).

> 🧭 **Quién fija la forma: el frontend.** Antes de escribir una línea de
> pipeline, abre `utils/ticketStats.js` del sistema heredado (Curso 01 · F7) y
> copia sus firmas, porque **son el contrato**:
>
> ```js
...
> ```
>
> El ejercicio 25 de esa fase pide que la vista use `/stats` **con fallback al
> cálculo local**. Fallback significa intercambiables: si tu endpoint devuelve
> otra forma, el fallback deja de ser un fallback y se vuelve una segunda
> implementación. Tres detalles del cliente que hay que clavar y que es fácil
> pasar por alto:
>
> 1. `countByStatus` devuelve un **objeto indexado por estado**, no un array.
> 2. `activeByAgent` cuenta solo `open` + `in_progress`, y los tickets sin
>    asignar **no se descartan**: caen en un bucket literal `"(sin asignar)"`
>    (es un `t.assignee || "(sin asignar)"`).
> 3. `resolvedPercent` redondea con `Math.round`.

**La división del trabajo:** el pipeline agrega en la forma que le es natural a
Mongo (arrays de `{clave, count}`); **el serializer de la frontera traduce a la
forma del cliente**. Es la misma disciplina de `serializeTicket` en la Fase 10,
aplicada a las métricas — y es lo que permite que `/stats` sea un reemplazo
directo del cálculo local, no un primo lejano.
...
async function computeStats(db) {
  const [result] = await db.collection("tickets").aggregate([
    { $facet: {
        ...
        // OJO: el frontend (Curso 01 · F7, activeByAgent) grafica SOLO carga
        ...
        activeByAgent: [
          ...
          // el cliente hace `t.assignee || "(sin asignar)"`: los tickets sin
          // asignar NO se descartan, se agrupan en un bucket propio. Si aquí
          // filtráramos por assignee, la dona perdería una porción entera.
          { $set: { agentKey: { $let: {
              vars: { a: { $ifNull: ["$assignee", ""] } },
              in: { $cond: [{ $eq: ["$$a", ""] }, "(sin asignar)", "$$a"] }
          } } } },
          { $group: { _id: "$agentKey", count: { $sum: 1 } } },
          { $sort: { count: -1 } },
          { $project: { _id: 0, agent: "$_id", count: 1 } }
        ],
        // total histórico por agente (todos los estados). No lo pinta el
        // cliente hoy; queda disponible como extensión del contrato. Acá sí
        // filtramos los sin asignar: es una métrica nueva y la definimos así.
        byAgent: [
        ...
        // insumos de resolvedPercent. Devolvemos total y done EN CRUDO y
        // redondeamos en el serializer con Math.round, para dar el mismo
        // entero que el cliente: el $round de Mongo redondea al par más
        // cercano y Math.round redondea hacia arriba — en un 62.5% eso es
        // un punto de diferencia, y "la métrica migrada tiene que dar el
        // mismo número" no admite un punto de diferencia.
        resolved: [
          { $group: { _id: null,
              total: { $sum: 1 },
              done: { $sum: { $cond: [
  ...
  // forma cruda, natural para Mongo. La traducción al contrato es del serializer:
  // { byStatus:[…], byPriority:[…], activeByAgent:[…], byAgent:[…],
  //   resolved:[{total,done}], resolution:[{…}] }
  return result;
}
```

#### `src/lib/serializers.js` — `serializeStats`, la frontera de las métricas

```js
// Traduce la forma cruda del pipeline a la forma que el frontend YA consume.
// Vive junto a serializeTicket porque es exactamente el mismo trabajo:
// decidir qué sale y con qué forma.
function serializeStats(raw) {
  const done = raw.resolved[0] || { total: 0, done: 0 };

  return {
    // objeto indexado por estado: reemplaza countByStatus() del cliente
    byStatus: raw.byStatus.reduce(function (acc, row) {
      acc[row.status] = row.count;
      return acc;
    }, {}),

    // misma forma y mismo orden que activeByAgent() del cliente
    activeByAgent: raw.activeByAgent,

    // Math.round, igual que el cliente (ver el comentario del pipeline)
    resolvedPercent: done.total === 0
      ? 0
      : Math.round((done.done / done.total) * 100),

    // --- de aquí abajo, extensiones que el cliente no calculaba ---
    byPriority: raw.byPriority.reduce(function (acc, row) {
      acc[row.priority] = row.count;
      return acc;
    }, {}),
    byAgent: raw.byAgent,
    resolution: raw.resolution[0] || null
  };
}
```

🔎 **Por qué `byPriority` también es objeto.** No tiene contraparte en el
cliente, así que podríamos devolverlo como array. Lo hacemos objeto por
**simetría con `byStatus`**: un mismo payload con dos convenciones para dos
conteos-por-campo es la clase de detalle que hace que alguien, en seis meses,
escriba el bug. La regla que queda: **conteo por campo → objeto indexado;
ranking ordenado → array**, porque en un ranking el orden es información y un
objeto no lo garantiza.
...
> ti: el dashboard heredado (Curso 01 · F7, `activeByAgent`) grafica **carga
...
> ⭐ **Este shape es el contrato de `/stats`,** y lo dicta el frontend. Las
> tres primeras claves **no se negocian**: son las tres funciones puras de
> `utils/ticketStats.js` (Curso 01 · F7), con su forma exacta, para que el
> ejercicio 25 de esa fase pueda hacer fallback al cálculo local sin tocar la
> vista. Las tres siguientes son extensiones que el cliente nunca calculó:
>
> ```js
> {
...
>   byPriority:      { high: 4, medium: 7, low: 2 },
>   byAgent:         [{ agent, count }],   // histórico completo, sin bucket
>   resolution:      { avgHours, maxHours, tickets } | null
> }
> ```
>
> 🧭 **La regla de oro, dicha entera:** la métrica migrada tiene que dar el
> mismo número **y con la misma forma**. Un número correcto dentro de una
> estructura distinta obliga a tocar la vista — y tocar la vista es
> exactamente lo que este curso promete no hacer.
...
> [`forense-fase-09.md`](forense-fase-09.md).
...
> `00-audit-contrato.md` ubica **aquí** (checklist de la Fase 9) la verificación
...
>
> ```bash
> git tag -a fase-09-aggregation -m "F9 cerrada: <el checklist, en una línea por ítem>"
> ```
>
...
> `ej/f09/17`, y un incidente resuelto en el par `inc/f09/<slug>-roto` /
...
|---|---|---|---|
| 09 | "El reporte por estado devuelve una sola línea" | Consultas | 🟠 |
