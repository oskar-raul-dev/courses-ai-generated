...
> 📌 **Esto es un extracto; la fixture oficial es el `db.json` completo del
> sistema heredado**, con tres tickets. No lo abrevies en tu seed real: copia
> el archivo tal cual, porque es el que produce los números que el frontend ya
> pinta.
>
...
async function main() {
  ...
  // ── 4. la OTRA referencia, la que no es un id: los usernames.
  // assignee, reporter y author no apuntan a un _id: apuntan a un username.
  // Sigue siendo una referencia, y sigue siendo tuya. Aquí NO lanzamos: la
  // fixture heredada ya viene con algunas rotas y romper el seed te dejaría
  // sin base. Se reportan, y tú decides (ver abajo).
  const known = new Set(users.map(function (u) { return u.username; }));
  const dangling = new Map();   // username -> [dónde aparece]

  function check(username, where) {
    if (username && !known.has(username)) {          // null = sin asignar, es válido
      if (!dangling.has(username)) dangling.set(username, []);
      dangling.get(username).push(where);
    }
  }

  tickets.forEach(function (t) {
    check(t.assignee, "ticket." + t._id + ".assignee");
    check(t.reporter, "ticket." + t._id + ".reporter");
  });
  comments.forEach(function (c) { check(c.author, "comment." + c._id + ".author"); });

  if (dangling.size > 0) {
    console.warn("\n⚠️  Usernames referenciados que NO existen en users:");
    dangling.forEach(function (where, username) {
      console.warn("   " + username + " → " + where.length + " referencia(s)");
    });
    console.warn("   Decide qué haces con esto y anótalo en DATA-MODEL.md.\n");
  }
...
### ⚠️ Y ahora corre el seed y lee la consola

Va a funcionar. Y va a avisarte de esto:
...
es una referencia y nunca miró.

🪞 **Tu instinto SQL dice** que esto no puede pasar, y tiene razón: en tu mundo
la base habría rechazado el INSERT. Pero fíjate en lo que acaba de ocurrir de
verdad: **el dato inconsistente no lo creó Mongo.** Llegó así. Mongo solo dejó
de ser el adulto responsable que lo detenía. Esa es la diferencia exacta entre
"la base garantiza la integridad" y "alguien garantiza la integridad" — y en
NoSQL ese alguien tiene tu nombre.

Hay dos decisiones defendibles y **tienes que elegir una y escribirla**:

- **Conservarlas tal cual.** Fidelidad al sistema real: reproduces el estado
  que vas a heredar, y en la Fase 5 vas a ver con tus ojos qué hace un
  `$lookup` contra una referencia rota (spoiler: array vacío, sin quejarse).
  Es la que recomendamos, porque el curso trata de mantener sistemas reales y
  los sistemas reales tienen basura dentro.
- **Sanearlas** creando los tres usuarios faltantes en el seed. Base limpia y
  consultas predecibles, pero dejas de reproducir el problema que te van a
  pedir arreglar.

Lo que **no** es defendible es la tercera opción, que es la que toma casi
todo el mundo: no haberse dado cuenta. Por eso el chequeo va en el seed y grita
en cada ejecución, en vez de vivir en un párrafo que se lee una vez.

> 🧭 **El patrón a memorizar:** cuando importes datos a Mongo, escribe el
> chequeo de integridad **en el mismo script del import**. No después, no "en
> un ticket", no en la revisión de código. El único momento en que alguien mira
> de verdad la consistencia de una migración es el momento en que la migración
> se lo pone delante.
...
> paga la capa API en la Fase 10. Está firmado en `00-audit-contrato.md`.
...
traducción en la frontera está decidida y firmada en `00-audit-contrato.md`, y
...
> [`forense-fase-01.md`](forense-fase-01.md).
...
30. **La decisión de las referencias rotas.** Corre el seed, lee el aviso, y escribe en `DATA-MODEL.md` cuál de las dos opciones tomas (conservar o sanear) con tu argumento en tres líneas. Después implementa la que elegiste. Si conservas: añade al `verify.js` una comprobación que cuente las referencias colgantes y **no falle** por ellas, pero las liste — la diferencia entre "roto" y "conocido". Si saneas: crea los tres usuarios en el seed con un `name` explícitamente marcado (`"(recuperado del legacy)"`), para que dentro de un año nadie crea que siempre estuvieron.
...
>
> ```bash
> git tag -a fase-01-mongo-en-30-min -m "F1 cerrada: <el checklist, en una línea por ítem>"
> ```
>
...
> `ej/f01/17`, y un incidente resuelto en el par `inc/f01/<slug>-roto` /
...
|---|---|---|---|
| 02 | "El ticket está en la base y mi script no lo encuentra" | Consultas | 🟡 |
