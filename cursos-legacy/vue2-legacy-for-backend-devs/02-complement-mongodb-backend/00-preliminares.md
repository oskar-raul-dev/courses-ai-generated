...
- el repo del backend **iniciado en git**, con el `.env` fuera de él y el
  primer tag de fase creado;
...
## 🏷️ El repo del backend, antes de la primera consulta

El compose, el `.env` de ejemplo, los scripts y el `SETUP.md` que vas a
escribir hoy ya son el proyecto. Entran a git ahora, en **su propio repo** —
...
git add .
git commit -m "f00: compose de Mongo 4.4 parametrizable + SETUP inicial"
```

Los commits llevan el prefijo de su fase (`f00:`, `f07:`) y los de ejercicio
además su número (`f07 ej22: índice ESR sobre status + createdAt`). Con eso,
`git log --oneline --grep '^f07'` te devuelve la fase entera.

**Las tres primeras líneas de ese `.gitignore` no son opcionales.** `.env`
lleva secretos —hoy poco interesantes, desde la Fase 11 un `JWT_SECRET` de
verdad— y un secreto que entra al repo ya no sale, aunque borres el archivo en
el commit siguiente: sigue vivo en la historia. Versiona en su lugar un
`.env.example` con las claves y sin los valores. El apéndice de seguridad
([`a04-seguridad.md`](a04-seguridad.md)) trata esto como regla dura, y con
razón. `mongo-data/` queda fuera por otro motivo: es la base entera en
formato WiredTiger, y no es algo que un repo deba cargar.

> 🧭 **¿Por qué otro repo y no el del frontend?** Porque la señal de éxito de
> este curso se mide con git y solo significa algo si el frontend tiene su
> propia historia: en la Fase 10 apagas json-server, cambias el `baseURL` y el
> **`git diff` del frontend tiene exactamente una línea**. En un monorepo ese
> `git diff` te devuelve además todo el backend que acabas de escribir, y la
> medición se convierte en humo. Lo firmado está en
> [`00-audit-contrato.md`](00-audit-contrato.md).

Al terminar la fase —con su checklist en verde, incluido el `mongorestore`
ensayado— marcas el hito:

```bash
git tag -a fase-00-preliminares -m "F0 cerrada: Mongo 4.4 en Docker con
compose parametrizable, mongosh y Compass conectados, playground con people,
ciclo mongoexport/mongoimport probado, mongodump + restore ensayado a otra
base, SETUP.md escrito."
```

Las dieciséis fases cierran igual. La convención completa —tags de ejercicio,
cómo guardar un `explain()` en el mensaje de un tag, y el par `-roto`/`-fix`
con que se cierra un incidente— está en
...
> [`forense-fase-00.md`](forense-fase-00.md).
...
>
> ```bash
> git tag -a fase-00-preliminares -m "F0 cerrada: <el checklist, en una línea por ítem>"
> ```
>
...
> `ej/f00/17`, y un incidente resuelto en el par `inc/f00/<slug>-roto` /
...
|---|---|---|---|
| 01 | "El contenedor está arriba y nadie conecta" | Operación | 🟢 |
