...
admitida y anunciada (`00-audit-contrato.md`).
...
> fabricarlo el cliente — la ruptura pactada. Y el terreno estaba preparado:
> en su Fase 3 el `authService` heredado ya pagó la deuda de devolver una
> Promise, y su ejercicio 18 ya ensayaba resolver el login contra la API
> (`GET /users?username=X`). Cambiar ese `login()` por una llamada a
> `POST /auth/login` es tocar una función, no reescribir la sesión.
...
8. El interceptor heredado, verificado: confirma en el frontend que el `Bearer` ya viajaba desde su Fase 2 (el `apiClient` con interceptor de request) y que ahora tu backend lo lee (una request en Network con el header presente: captura). Y verifica que el mensaje de login es uniforme: "usuario inexistente" y "clave mala" devuelven EXACTAMENTE el mismo 401; si no, arréglalo.
...
>
> ```bash
> git tag -a fase-11-auth-real-y-pago-de-deudas -m "F11 cerrada: <el checklist, en una línea por ítem>"
> ```
>
...
> `ej/f11/17`, y un incidente resuelto en el par `inc/f11/<slug>-roto` /
