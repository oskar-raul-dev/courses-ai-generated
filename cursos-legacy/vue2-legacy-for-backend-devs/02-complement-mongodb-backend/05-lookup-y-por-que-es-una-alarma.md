...
2. **Siempre es LEFT.** Sin match, array vacío `[]` — nunca desaparece el
   ...
   > el resultado. En SQL nunca tuviste que distinguirlas porque la FK impedía
   > la segunda. Aquí las dos conviven, y solo tú sabes cuál estás mirando.
   >
   > Es exactamente la razón por la que el chequeo de integridad vive en el
   > seed y no en la consulta: **para cuando llegas al `$lookup`, el dato roto
   > ya es invisible.**
...
> [`forense-fase-05.md`](forense-fase-05.md).
...
16. `$unwind` traicionero: sobre el resultado del ejercicio 1, aplica `$unwind` sin `preserveNullAndEmptyArrays`, cuenta, y encuentra cuántos tickets desaparecieron. Escribe el bug report como si un compañero lo hubiera cometido en producción (2019, caso real en miles de equipos).
17. La cadena: en `soporte_v1`, el detalle completo (ticket + estado + prioridad + asignado + reportador + comentarios + autor de cada comentario — este último es un lookup con pipeline anidado). Cuéntale las etapas. Guarda el engendro: es la pieza central del ejercicio 24.
18. Sincronización dirigida: implementa `renameAgent(username, newName)` con las dos escrituras del capítulo + logging de cuántos tickets tocó. Mátalo a la mitad (un `process.exit` entre ambas) y fotografía el drift con una consulta.
19. Escribe `scripts/reconcile-assignee-names.js` completo (detecta + repara + reporta, idempotente). Úsalo para sanar el drift del ejercicio 17. Prográmalo mentalmente: ¿cada cuánto correría en tu tolerancia declarada?

**🟠 Difícil (20–28)**

20. **El laboratorio central (parte 1):** cronometra las tres formas del listado (20 tickets + reporter) a 4 escalas de la colección izquierda: 1k, 10k, 100k, y con `limit` 20 vs 500. Tabla completa (forma × escala × limit) en `DATA-MODEL.md`.
...
29. Radiografía del nested-loop: demuestra empíricamente que el costo del `$lookup` básico es ~lineal en la izquierda. Corre la misma unión con izquierda de 100, 1k, 10k, 100k (derecha constante), grafica/tabula tiempo vs tamaño, ajusta a ojo la curva.
...
>
> ```bash
> git tag -a fase-05-lookup-y-por-que-es-una-alarma -m "F5 cerrada: <el checklist, en una línea por ítem>"
> ```
>
...
> `ej/f05/17`, y un incidente resuelto en el par `inc/f05/<slug>-roto` /
...
|---|---|---|---|
| 05 | "El tablero tarda cuatro segundos y antes iba bien" | Modelado | 🟠 |
