...
```
        ...
        asyncData(context)                        mounted()
          └─ GET /activity?ticketId=1               └─ socketService.on("activity:created", ...)
...
export default {
  ...
  mounted: function () {
    ...
    socketService.on("activity:created", this.onNuevaActividad);
    this.isLive = socketService.isConnected();
  },

  // baja simétrica: todo lo que se suscribe, se desuscribe (regla de F8).
  beforeDestroy: function () {
    socketService.off("activity:created", this.onNuevaActividad);
  },
...
socketService.emit("activity:created", evento); // el server lo rebota (broadcast, F8)
```
...
```js
socket.on("activity:created", function (ev) {
  socket.broadcast.emit("activity:created", ev); // a todos menos al emisor
});
```
...
23. **El cliente mentiroso, versión historial.** Desde la consola, emite a mano
    `socket.emit("activity:created", {ticketId:1, type:"status_change", to:"closed", actor:"HACKER"})`.
...
>
> ```bash
> git tag -a fase-nx4-pagina-ssr-nueva -m "NX4 cerrada: <el checklist, en una línea por ítem>"
> ```
>
> Los commits de la fase llevan su prefijo (`nx4: …`) y los de ejercicio su
> número (`nx4 ej17: …`). Si un ejercicio merece su propio marcador va en
> `ej/nx4/17`, y un incidente resuelto en el par `inc/nx4/<slug>-roto` /
