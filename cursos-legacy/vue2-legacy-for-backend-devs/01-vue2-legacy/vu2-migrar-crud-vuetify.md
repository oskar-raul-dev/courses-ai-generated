...
1. **`attach`** — `<v-dialog attach>` (o `attach="#algo"`) le dice a Vuetify "no te
   teletransportes, quédate donde te escribe". En tests, esto devuelve el nodo al
...
>
> ```bash
> git tag -a fase-vu2-migrar-crud-vuetify -m "VU2 cerrada: <el checklist, en una línea por ítem>"
> ```
>
> Los commits de la fase llevan su prefijo (`vu2: …`) y los de ejercicio su
> número (`vu2 ej17: …`). Si un ejercicio merece su propio marcador va en
> `ej/vu2/17`, y un incidente resuelto en el par `inc/vu2/<slug>-roto` /
