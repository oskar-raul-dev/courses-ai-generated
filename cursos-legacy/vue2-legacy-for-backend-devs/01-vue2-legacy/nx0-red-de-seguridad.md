...
──────────────────────────────────────────────
[ ] ¿`npm run test:unit` está TODO verde? Cero .skip, cero rojos
    "que ya estaban ahí".
[ ] ¿`git status` limpio? La migración arranca desde un commit conocido.
[ ] ¿Existe el tag `pre-nuxt`? Es el punto al que vuelves cuando te
    pierdas. Y con SSR te vas a perder.
...
Los tres primeros puntos son los mismos de Q0 y VU0, y el del tag es literal:

```bash
git tag pre-nuxt
```

Un tag ligero, un segundo, cero espacio — y el único camino de vuelta cuando
NX2 te deje treinta tests rojos y `window is not defined` en la consola del
servidor. Convive con el `fase-nx0-red-de-seguridad` que crearás al cerrar esta
fase: son dos nombres para el mismo commit, y dicen cosas distintas. Uno dice
"terminé la fase"; el otro, "acá es donde vuelvo".

El último punto de la lista es tu inventario de deuda: cada pieza que toca `window` es una
...
> [`forense-ruta-nx.md`](forense-ruta-nx.md).
...
>
> ```bash
> git tag -a fase-nx0-red-de-seguridad -m "NX0 cerrada: <el checklist, en una línea por ítem>"
> ```
>
> Los commits de la fase llevan su prefijo (`nx0: …`) y los de ejercicio su
> número (`nx0 ej17: …`). Si un ejercicio merece su propio marcador va en
> `ej/nx0/17`, y un incidente resuelto en el par `inc/nx0/<slug>-roto` /
