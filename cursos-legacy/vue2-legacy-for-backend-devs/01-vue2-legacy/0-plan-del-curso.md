...
producción) está en **`0-ESTRUCTURA-CURSO.md`**.

### 🍃 Y la otra mitad del sistema

Este curso construye el frontend y lo deja hablándole a un mock. Todo lo que
aquí se difiere con un *"esto lo haría un backend real"* —el `reporter` que el
navegador inventa, el evento de socket que emite el cliente, el doble "tomar"
sin candado, las métricas calculadas en el navegador— **se paga en el curso
hermano**: [`../02-complement-mongodb-backend/`](../02-complement-mongodb-backend/README.md),
que reemplaza json-server por MongoDB 4.4 + Express sin que ninguna vista se
entere.

No hace falta hacerlo para terminar este. Pero si tu trabajo incluye el
backend, es la continuación natural, y el contrato que une los dos cursos está
firmado en
...
ejecución. Ver la ruta NX, de `nx0-red-de-seguridad.md` a
...
Detalle de cada una: los archivos `q0`–`q4` (Quasar), `vu0`–`vu4` (Vuetify) y
`nx0`–`nx4` (Nuxt). El índice completo con su orden está en `README.md`; la
arquitectura y las reglas de exclusión, en `0-ESTRUCTURA-CURSO.md`.
