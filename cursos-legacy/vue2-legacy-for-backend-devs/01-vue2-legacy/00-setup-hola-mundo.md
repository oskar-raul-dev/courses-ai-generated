...
- el proyecto **versionado en git desde el minuto cero**, con su primer commit
  y su primer tag de fase;
...
Antes de escribir una línea de Vue, el proyecto entra a git. No es ceremonia:
en este curso vas a romper cosas a propósito —los ejercicios están diseñados
...
De ahí en adelante, los commits llevan el prefijo de la fase —`f00:`, `f01:`,
`f04:`— y los de ejercicio además su número: `f04 ej17: filtro por prioridad`.
Un prefijo, nada más; con eso `git log --oneline --grep '^f04'` te devuelve
todo lo que hiciste en la Fase 4.

Vue CLI ya te dejó un `.gitignore` correcto; antes del primer commit confirma
que `node_modules/` y `dist/` están adentro y que `package-lock.json` **no**
lo está — esa foto exacta del árbol sí se versiona, por lo que acabas de leer
en la disección de arriba.

Y al terminar la fase —con el checklist de la sección "✅ Qué queda listo al
terminar" en verde, no antes— marcas el hito con un tag anotado:

```bash
git tag -a fase-00-setup-hola-mundo -m "F0 cerrada: Node 14 con NVM y .nvmrc,
proyecto Vue CLI con las dependencias del curso, editor con formateo al
guardar, Bootstrap importado, mock Stubby en :8888, Hello World completo."
```

A partir de acá cada fase cierra igual: commit, tag `fase-NN-…`, y a la
siguiente. Doce fases después vas a poder correr `git tag -l 'fase-*'` y ver
el curso entero en una pantalla. La convención completa —por qué el backend va
en otro repo, cómo etiquetar ejercicios y cómo dejar un incidente resuelto con
su par de tags— está en
...
> eso. Los tags son punteros a un commit: no ocupan espacio, no cambian nada y
> se borran con `git tag -d`. La convención se lee en diez minutos y te ahorra
> la tarde en que el ejercicio 22 te deje el proyecto irreconocible.
...
|---|---|---|---|
| 01 | "Clonaste el repo y no me arranca, a ti sí te funciona" | Build | 🟢 |
