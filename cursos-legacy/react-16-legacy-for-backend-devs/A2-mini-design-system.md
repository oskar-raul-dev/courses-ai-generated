...
> Lo usan: Fases 0, 4 y 9 · Prerrequisito: `A1-bootstrap-4-y-sass.md`
>
> **Material de consulta rápida, no lectura secuencial.** Lo abres cuando una
> fase te manda acá o cuando estás por escribir estilos y no sabes dónde va
...
más útil de entender cuando mantienes: **un puñado de archivos Sass que
...
porque negocio lo pidió, quieres cambiarlo **en un solo lugar** y que se propague
...
> 💡 **Convención de import de parciales.** Escribes `@use "./tokens"` (sin el
> guion bajo y sin la extensión). Sass resuelve `_tokens.scss` solo. Mantén el
...
- **Para nuestros propios parciales** (`_tokens`, `_functions`, `_mixins`) usamos
  **`@use`**. Es código nuestro, moderno, y nos da namespaces explícitos: cuando
  ves `tokens.$primary` sabes de dónde sale la variable sin adivinar. Es la forma
...
- **Para Bootstrap 4 usamos `@import`.** Y no es pereza: **Bootstrap 4.6.2 está
  ...
  *antes* de importar Bootstrap— depende de ese scope global. Con `@use`, las
  ...
  compila como esperas y contradice cómo se customiza esta versión.
...
> antes que cualquier `@import`. Si mezclas el orden, Sass tira
...
> agrupados. Si te pelea, separa: un archivo `_vendor.scss` con los `@import`
...
> 90% de las veces el problema no es especificidad: es **orden**. Si redefines
...
> el color, lo primero que miras es el orden de imports, no la especificidad.
...
> pasas entre llaves. Sin `@content`, el `@include above("md") { ... }` compila
...
> escribir un mixin, pregúntate si BS4 ya lo resuelve. `status-badge` casi se
...
> 1px, tienes saltos de layout que nadie entiende.
...
- 🟢 **1.** Agrega un token `$radius-pill: 50rem` y úsalo para que los badges de
  estado sean redondeados. ¿Dónde va el token y dónde el uso?
- 🟢 **2.** El verde de "abierta" cambió a `#2e9e4f`. Cámbialo en **un solo
  lugar** y verifica que se propaga a `.badge-success` de Bootstrap y a
  `number-state("available")`. ¿Por qué basta un lugar?
- 🟡 **3.** Escribe una function `status-label-color($status)` que devuelva
  `#fff` para estados oscuros y `#212529` para `"reserved"` (fondo ámbar claro).
  Intégrala en `status-badge`.
- 🟡 **4.** Convierte un `@media (min-width: 992px)` hardcodeado de un componente
  a `@include above("lg")`. Confirma que el resultado compilado es idéntico.
- 🟡 **5.** Un compañero puso `$theme-colors` **después** del `@import` de
  Bootstrap y "no anda". Reproduce el bug, explica la causa en una frase y
  corrígelo.
- 🟠 **6.** Un mixin `above("xxl")` no aplica nada y no da error. Diagnostica por
  qué (pista: `map-has-key`) y agrega el manejo correcto.
- 🟠 **7.** Refactoriza tres bloques `.scss` que repiten el filete superior de
  color hacia el mixin `raffle-accent`. Mide cuántas líneas se eliminaron.
- 🟠 **8.** Escribe un test manual: cambia `$spacer` y documenta qué utilidades
...
- 🔴 **9.** Un mixin de breakpoint sin `@content` está tragándose estilos
  responsive en producción y en dev "parece andar". Reproduce, encuéntralo, y
  escribe una nota de prevención para el equipo.
- 🔥 **10.** *(Puente a moderno)* Investiga cómo se pasarían overrides a Bootstrap
  **5** con `@use "bootstrap" with (...)` y explica por qué ese patrón **no** es
...
- 🔥 **11.** *(Puente a moderno)* dart-sass emite deprecation warnings al mezclar
  `@use` con dependencias basadas en `@import`. Captura uno de esos warnings en
  la consola de build y explica qué te está avisando y por qué lo aceptamos acá.
...
> respuesta vive acá: token, function o mixin. Si te encuentras hardcodeando un
> `#dc3545` o un `@media (min-width: 768px)` en un componente, para y vuelve a
...
>
> ```bash
> git tag -a apendice-a2-mini-design-system -m "A2: _tokens.scss, functions y mixins propios, y los componentes pasados
> a las clases del sistema."
> ```
>
> Los commits llevan su prefijo (`a2: …`) y los de ejercicio su número
> (`a2 ej3: …`). La convención completa —tags de fase, de ejercicio y de
