...
real con sus propios malos días, y quien lo mantiene necesita
...
- [ ] `json-server` corriendo en el puerto `3001`, sirviendo `db.json`
      (`raffles`, `numbers`, `participants`, `settlements`, `users`) con CRUD
      funcional desde la app en `3000`.
- [ ] Tres **rutas propias** montadas sobre json-server que el CRUD automático
      no puede dar: `GET /raffles/:id/numbers`,
      `POST /raffles/:id/numbers/:number/reserve` y
      `POST /raffles/:id/numbers/:number/sell` (con `409` ante venta
      duplicada). Nadie las consume hasta la Fase 5; quedan listas desde ya.
...
- **Depurar el build de producción** (source maps, comparar ambientes) →
  `A13-depurar-el-build-de-produccion.md`. Acá simulamos el backend áspero; el
  día que un bug no se reproduzca en desarrollo, ese apéndice es el camino.
...
en producción: el CRUD es relativamente estable (es tu propio
backend), pero la lotería es un servicio externo con su propio SLA, su
propia latencia y sus propios apagones. Si mezclas el caos de ambos en el
...
> 🧭 **Y el ambiente que este mock simula.** `CHAOS_LEVEL` no solo inyecta
> fallos: define un **ambiente**. `off` es tu desarrollo, `low` se parece a un
> UAT sano y `high` a una producción con un mal día. Esa correspondencia se usa
> en serio en `A13-depurar-el-build-de-produccion.md` §5, donde los tres niveles
> más el build de CRA arman los tres ambientes contra los que vas a comparar
> cuando alguien reporte un «funciona en UAT pero no en PROD».
...
> 📝 **Nota de época.** En producción este "modo caos" nunca existió como
...
Fíjate que las **claves** (`raffles`, `numbers`, `status`) van en inglés,
...
function createChaosMiddleware(level = "off") {
  ...
  return function chaosMiddleware(req, res, next) {
    ...
    setTimeout(() => {
      ...
      if (type === "serverError") {
        // 💸 mensaje genérico a propósito: en producción vas a ver
        // errores igual de poco útiles. Practica con esto.
...
### `mock/server.js` — json-server + caos, puerto 3001

```javascript
// mock/server.js
const jsonServer = require("json-server");
const { createChaosMiddleware } = require("./chaosMiddleware");
const { rafflesNumbersRouter } = require("./rafflesNumbersRouter");
...
server.use(jsonServer.bodyParser);              // necesario para leer req.body en las rutas propias
server.use(createChaosMiddleware(level));       // el caos va ANTES de todo lo que responda
server.use(rafflesNumbersRouter(router.db));    // rutas propias del dominio (ver abajo)
server.use(router);                             // y al final el CRUD automático
...
> ⚠️ El orden importa, y ahora importa dos veces. El middleware caos se monta
> **antes** que todo lo que pueda responder: si lo pones después, nunca se
> ejecuta porque alguien ya contestó. Y las rutas propias van **antes** que
> `router`, porque json-server responde a cualquier cosa que empiece con un
> nombre de colección — si lo montas primero, se traga `/raffles/1/numbers` y
> tus rutas nunca corren.

### Las tres rutas que json-server no puede darte

`json-server` te regala el CRUD de cada colección, pero el dominio necesita tres
operaciones que no son CRUD y que ninguna convención de query resuelve bien:
**leer el tablero de una rifa**, **reservar un número** y **venderlo con
unicidad estricta**. Esas se escriben a mano.

Las escribimos ahora, en la Fase 3, aunque nadie las consuma hasta la Fase 5.
La razón es la misma por la que el mock de lotería ya nace con caos: **la fase
que enseña venta concurrente tiene que poder concentrarse en la concurrencia**,
no en armar su backend. Acá el backend queda listo, feo y honesto.

```javascript
// mock/rafflesNumbersRouter.js
// Rutas propias del dominio, montadas sobre la misma base de datos que
// json-server (lowdb). Se le pasa `router.db` para escribir en el mismo
// db.json, no en una copia.
const express = require("express");

/**
 * @param {object} db - la instancia lowdb de json-server (router.db)
 * @returns {import('express').Router}
 */
function rafflesNumbersRouter(db) {
  const routes = express.Router();

  // GET /raffles/:raffleId/numbers → el tablero de esa rifa.
  // Con json-server puro sería /numbers?raffleId=1, que funciona pero deja
  // la forma del recurso a merced de la query. La ruta explícita documenta
  // la intención: los números pertenecen a la rifa.
  routes.get("/raffles/:raffleId/numbers", (req, res) => {
    const raffleId = Number(req.params.raffleId);
    const numbers = db.get("numbers").filter({ raffleId }).value();
    res.json(numbers);
  });

  // POST /raffles/:raffleId/numbers/:number/reserve
  // 409 si el número no está disponible. La reserva NO guarda expiración en
  // el backend: quién la expira es el frontend (Fase 5 con setTimeout, Fase 6
  // con un epic). 💸 Deuda deliberada: un backend serio expiraría del lado
  // del servidor, porque el cliente puede cerrar el navegador y dejar el
  // número bloqueado para siempre. Está así para que el problema se vea.
  routes.post("/raffles/:raffleId/numbers/:number/reserve", (req, res) => {
    const raffleId = Number(req.params.raffleId);
    const record = db.get("numbers").find({ raffleId, number: req.params.number });
    const current = record.value();

    if (!current) return res.status(404).json({ message: "Ese número no existe en la rifa" });
    if (current.status !== "available") {
      return res.status(409).json({ message: "Ese número ya no está disponible" });
    }
    record.assign({ status: "reserved" }).write();
    res.json(record.value());
  });

  // POST /raffles/:raffleId/numbers/:number/sell
  // Acá vive la unicidad estricta, y por eso este handler es el más
  // importante del mock: es la última línea de defensa contra vender dos
  // veces. Devuelve 409 si el número ya está sold — que es exactamente el
  // error que la Fase 5 tiene que aprender a revertir.
  routes.post("/raffles/:raffleId/numbers/:number/sell", (req, res) => {
    const raffleId = Number(req.params.raffleId);
    const record = db.get("numbers").find({ raffleId, number: req.params.number });
    const current = record.value();
    ...
    if (current.status === "sold") {
      return res.status(409).json({ message: "Ese número ya fue vendido" });
    }
    record
      .assign({ status: "sold", participantId: req.body.participantId ?? null })
      .write();
    res.json(record.value());
  });
...
module.exports = { rafflesNumbersRouter };
```

> 🧠 **Por qué el `409` y no un `400`.** Un `400` dice "tu petición está mal
> formada"; un `409 Conflict` dice "tu petición está bien, pero el estado del
> recurso no permite lo que pides". La diferencia importa para el frontend:
> ante un `400` revisas tu código, ante un `409` le dices al usuario que elija
> otro número. La Fase 5 le da un `type: 'conflict'` propio en el error legible
> precisamente por esto.

> ⚠️ **Estas rutas quedan protegidas automáticamente.** `PROTECTED_ROUTES`
> incluye `/raffles` y la comprobación es `startsWith`, así que
> `/raffles/1/numbers/0347/sell` entra en la baraja del `401`. Eso es lo que
> hace posible el ejercicio de Fase 5 donde el token se cae en mitad de una
> venta.

### `lottery/server.js` — mock de lotería, puerto 3002

```javascript
// mock/lottery/server.js
...
En la Fase 2 el `authSlice` arrancaba siempre vacío: recarga la página y
...
> es **legible por cualquier script** de la página (si te comes un XSS, se
...
> 🔴 sobre esto); y **no se limpia solo**: si no llamas `clearSession()`,
> el token sobrevive al cierre del navegador.

**Trade-off (por qué `localStorage` y no otra cosa):** elige
`localStorage` porque es lo que el sistema usa desde 2019 y lo que Fase 2
...
Con `CHAOS_LEVEL=high`, loguéate y haz clic repetidas veces en algo que
pegue a `/raffles`. Tarde o temprano vas a comer un 401 inyectado: la app
debería **sacarte a `/login` sola**, con el `localStorage` limpio. Vuelve a
entrar y confirma que la sesión persiste tras recargar (F5) — eso valida
...
3. **"Me saca a login al azar y no entiendo por qué."** Es el 401
   ...
   es un bug: es la práctica. Baja `CHAOS_LEVEL` a `low` u `off` para
   trabajar tranquilo.
4. **"Recargo la página y sigo logueado aunque el token ya no vale."** La
   ...
   interceptor de 401.
5. **"Confunde en qué puerto está cada mock."** Verifica `baseURL` contra
   la tabla: `3000` app, `3001` CRUD, `3002` lotería.

### Pieza forense de esta fase — Simular caos con Express: cómo reacciona la SPA

Con `CHAOS_LEVEL=high` en `3001` y la app corriendo, abre Network en
Chrome DevTools y reproduce, anotando qué ve el usuario:

1. **500 puro:** filtra Network por status `500`. ¿La UI avisa algo o
   falla en silencio?
2. **Timeout:** busca una petición "pendiente" más de 5s. ¿Tu axios la
   corta? ¿Qué ve el usuario?
3. **Malformado:** fuérzalo (sube `failRate` a 1 temporalmente) y mira si
   el componente explota al parsear o lo maneja con gracia.
4. **401 y correlación:** provoca un 401, confirma que la app te saca a
   `/login`, y **cruza el `request-id`** del `console.warn` del
...
> haz que `pickFailureType()` devuelva siempre el tipo que quieres.
> Revierte después.

Esta pieza forense es el insumo de los incidentes **07** y **08** de
...
**🟢 Fácil (1–8)**
1. Levanta `json-server` en `3001` y confirma con `curl` que `GET /raffles`
   devuelve la rifa de ejemplo.
2. Cambia `CHAOS_LEVEL` a `off` y confirma respuestas en menos de 50ms.
3. Cambia `CHAOS_LEVEL` a `high` y cuenta cuántas de 20 peticiones a
   `/raffles` fallan.
4. Levanta el mock de lotería en `3002` y confirma con `curl` que
   `GET /results/1` devuelve el JSON esperado.
5. Con la sesión iniciada, recarga la página (F5) y confirma que sigues
   logueado (validación del pago de persistencia).
6. Haz logout y confirma en DevTools → Application → Local Storage que la
   clave `rifas.auth` desapareció.
7. Agrega `console.log(level)` en `chaosMiddleware.js` y confirma que
   coincide con tu `.env`.
8. Explica en un comentario por qué el caos debe montarse antes del
   `router` de json-server.

**🟡 Intermedio (9–17)**
9. Provoca un 401 con `CHAOS_LEVEL=high` y confirma que la app te
   redirige a `/login` sola.
10. Mete a mano un JSON corrupto en la clave `rifas.auth` de localStorage
    y confirma que la app arranca como "sin sesión" en vez de reventar
    (prueba que `readSession()` devuelve `null`).
11. Agrega un nivel `CHAOS_LEVEL=extreme` a `CHAOS_LEVELS` con
    `failRate: 0.4` y pruébalo.
12. Quita el `timeout` de `apiClient` y documenta, con capturas de
    Network, qué le pasa a la UI bajo `high`.
13. Verifica que el 401 **no** se inyecta sobre una ruta pública (agrega
    una ruta pública de prueba y comprueba que `isProtectedRoute()` la
    excluye).
14. Correlaciona un 401: encuentra el `request-id` en el `console.warn` del
    interceptor y búscalo en el log del servidor mock.
15. Agrega una ruta `/health` a json-server que **no** pase por el caos,
    para health-checks.
16. Documenta en un comentario la diferencia entre corrección mínima
    (logout en interceptor) y refactor (refresh token) para el 401.
17. Cambia `CHAOS_LOTTERY_LEVEL` a `off` y confirma que `/results/1`
    responde siempre rápido y sin fallos.

**🟠 Difícil (18–24)**
18. Reproduce el error común #1 a propósito (caos después del router) y
    confirma con Network que nunca se activa.
19. Escribe un script Node que dispare 50 peticiones concurrentes a
    `/raffles` bajo `high` y cuente cuántas cayeron en cada tipo
    (`ok`/`unauthorized`/`serverError`/`timeout`/`malformed`).
20. Diagnostica: "a veces el dashboard muestra `undefined` en vez del
    nombre de la rifa". Formula una hipótesis con lo que sabes del caos y
    confírmala con Network.
21. Agrega al interceptor de response un log cuando una respuesta tarde
    más de 1000ms (instrumentación de latencia).
22. Haz que el `failRate` del tipo `unauthorized` dependa de la ruta (por
    ejemplo, `/settlements` nunca da 401) y explica un caso de uso real.
23. Sin mirar `chaosMiddleware.js`, deduce a partir de Network qué
    `failRate` aproximado está configurado.
24. Diagnostica un `PATCH /numbers/...` que cae en `malformed`: ¿el número
    queda en estado inconsistente en `db.json`?

**🔴 Muy difícil (25–30)**
25. **Sesión zombi entre pestañas:** abre la app en dos pestañas, loguéate
    en ambas, y haz logout en una. Confirma que la otra pestaña sigue
    mostrando sesión activa en su store de Redux aunque el `localStorage`
    ya se limpió. Documenta por qué pasa y propón (sin implementar) cómo
    sincronizarlas.
26. Diseña y documenta un incidente propio (síntoma + reproducción + causa
    raíz) combinando un 401 inyectado y un componente que no maneja el
    redirect.
27. Correlación punta a punta: arma el flujo completo que cruza un 401 en
    Network → `request-id` en consola → `request-id` en el log del mock,
    y escríbelo como procedimiento reproducible.
28. Extiende `chaosMiddleware` con latencia de distribución no uniforme
    (muchas rápidas, pocas muy lentas) y justifica por qué se parece más a
    un backend real.
29. Investiga qué pasa si un 401 llega **mientras** una segunda petición
    protegida ya está en vuelo: ¿se despacha `logout` dos veces? ¿importa?
30. Semilla para el cuaderno de incidentes: propón un incidente 🟡 de
    "CORS" para la Semana 1, considerando los tres orígenes (`3000`,
    `3001`, `3002`).

**🔥 Opcionales**
- 🔥 Agrega `POST /_chaos` para cambiar `CHAOS_LEVEL` en caliente sin
  reiniciar (adelanto de la deuda de la sección 5).
- 🔥 Sincroniza el logout entre pestañas con el evento `storage` de
  `window` (resuelve el ejercicio 🔴 #25 de verdad).
- 🔥 Dockeriza ambos mocks en el `docker-compose.yml` de la Fase 0.
...
**Video / apoyo**
- Búsqueda sugerida en YouTube: "json-server tutorial crash course" — confirma que corresponda a la versión 0.x (json-server tuvo cambios grandes en su v1).
...
> ⚠️ Verifica siempre la versión de la documentación: `json-server` tuvo
...
> confírmalos antes de recomendarlos al grupo.
...
> **La señal de que quedó bien:** si apagas `json-server` a mitad de una
...
>
> ```bash
> git tag -a fase-03-mock-api-express-caos -m "F3 cerrada: <el checklist, en una línea por ítem>"
> ```
>
...
- **Cambiar `CHAOS_LEVEL` en caliente** (un `POST /_chaos`) aparece como ejercicio
  🔥 recién en Fase 7, cuando el mock nace acá. → Decidir si el ejercicio se
  adelanta a esta fase.
- **El pago de deuda de persistencia no tiene smoke.** Que la sesión sobreviva al
  F5 se verifica a mano. → Test de Fase 10.
- **El malformado (200 con body inesperado) es el caos más interesante y el peor
  cubierto.** Se provoca subiendo `failRate` a mano. → Ejercicio 🟠 con un flag
  propio.
...
- **07** — *A veces no carga y no dice nada* · Integración · 🟡. El ticket vago
  canónico del curso. Sale del timeout de §6: la petición queda pendiente más de
  5 s y la UI no dice nada. Enseña que "a veces" es un dato, no un defecto del
  reporte.
- **08** — *Me saca a login al azar mientras estoy trabajando* · Integración · 🟡.
  Sale del 401 aleatorio del middleware de caos cruzado con el interceptor global
  que se estrena en esta fase.
