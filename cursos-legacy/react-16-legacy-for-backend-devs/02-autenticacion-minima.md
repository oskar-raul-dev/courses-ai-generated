...
- **Conectar `demoRaffles` / `demoParticipants` al backend** → la deuda 💸 de datos hardcodeados de Fase 1 sigue abierta y se paga en **Fase 4** (Fase 3 monta el backend; Fase 4 es la que tiene store para consumirlo). No la toques en esta fase.
...
> Enlaza con los incidentes **05** y **06** de `cuaderno-incidentes.md` (Redux
> DevTools + correlación por `requestId`). Los dos se resuelven con lo que sabes
...
- `A6-redux-clasico-vs-toolkit.md` — **este es el primer Redux del curso**; si vienes de Redux clásico, el mapa de equivalencias (`createStore` ↔ `configureStore`, `switch` ↔ `createSlice`) está ahí y ahorra media hora de desconcierto.
- `A1-bootstrap-4-y-sass.md` §5 — las clases de formulario (`form-group`, `form-control`, `invalid-feedback`) que usa `LoginForm`.
...
>
> ```bash
> git tag -a fase-02-autenticacion-minima -m "F2 cerrada: <el checklist, en una línea por ítem>"
> ```
>
...
- **La password viaja por query string.** Está marcada 💸 como deuda doble, pero
  fuera de un curso esto no es deuda: es un hallazgo de seguridad. → Agregar una
  ⚠️ que diga exactamente eso, para que nadie lo copie pensando que es un atajo
  aceptable.
- **`useSelector` que devuelve un objeto nuevo en cada render** es el error común
  #4 y el más transferible de los cuatro, pero se queda en descripción. →
  Ejercicio 🟠 de diagnóstico con el Profiler.
- **El interceptor de respuesta todavía no existe.** Acá solo se adjunta el token;
  el manejo global del 401 se paga en Fase 3. → Verificar que el pago quede
  marcado como **💸 Pago de deuda** allá.
- **Correlación por `requestId` sin contraparte de servidor.** El header se emite
  y se lee, pero no hay nada que lo consuma del otro lado. → Ejercicio 🔥: un
  endpoint del mock que devuelva el log de un `req-id`.
...
- **05** — *Las peticiones salen sin token y el servidor las rebota* · Autenticación · 🟡.
  Sale del error común #1 (interceptor mal registrado o registrado después de la
  primera petición).
- **06** — *Cerré sesión pero la pantalla sigue mostrando mi usuario* · Autenticación · 🟡.
  Cruza el `authSlice` sin persistencia 💸 con el loop de redirección del error
  común #2. Se resuelve con Redux DevTools antes que con código.
