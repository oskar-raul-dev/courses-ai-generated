...
- [ ] Venta con el thunk `sellNumber` sobre el endpoint `POST /raffles/:id/numbers/:number/sell`, con unicidad estricta y un **nuevo `type: 'conflict'`** sumado al esquema de error legible, para la carrera perdida (`409`).
...
- **Participantes reales** (a quién se le vendió, contacto, forma de pago) → fuera del curso base; queda como 🔥 y está anotado en `A12-mapa-de-deuda-tecnica.md`. Acá una venta registra a lo sumo un `participantId` placeholder; no hay formulario de comprador todavía.
...
El CRUD de Fase 4 tenía una propiedad cómoda que no valoras hasta que la pierdes: **las operaciones eran independientes.** Crear la rifa A no compite con crear la rifa B; cada thunk vivía en su burbuja. La venta rompe eso de raíz. El recurso en disputa —el número `0347` de la rifa 1— es **único y compartido.** Dos operaciones sobre el mismo número no son independientes: son rivales. Una tiene que ganar y la otra tiene que perder de forma limpia, y el store no puede quedar mintiendo que ambas ganaron.

📝 **De dónde viene este problema en este sistema.** La venta optimista con rollback es de 2021, cuando el negocio pasó de un kiosco a tres y aparecieron los primeros números vendidos dos veces (`00-historia-del-sistema.md` §3, era 2). La solución de entonces resolvió el 99% de los casos; el 1% restante es el incidente ⭐ 11 del cuaderno, y es lo que la Fase 6 termina de cerrar con epics.
...
La versión de esta fase: cuando `reserveNumber` sale bien, arrancas un `setTimeout` que, al vencer, despacha `reservationExpired`. Simple. Y con tres trampas que vas a pisar, porque son justo las que el epic resuelve gratis:
...
2. **Si el componente se desmonta, el timer sigue corriendo.** Navegas fuera del tablero y el `setTimeout` sobrevive, apuntando a un store que quizá ya cambió de rifa. Es un **memory leak de timer**, primo hermano del leak de suscripción que vas a cazar en Fase 6.
3. **Si cambias de rifa, los timers de la anterior siguen agendados.** Sin una limpieza explícita, se acumulan como pestañas del navegador un viernes.
...
Fase 4 dejó `RaffleTable` en clase (`connect()`) y `RaffleForm` en hooks para mostrar el código mezclado real del sistema. Fase 5 es toda **funcional con hooks**: el tablero y sus celdas se benefician de `useMemo`/`React.memo`/`useSelector` puntual de una forma que en clase sería más verbosa y menos clara. No fuerzo una clase donde no aporta —eso sería solemnidad de manual, no pedagogía—. Si quieres seguir aceitando la convivencia, el ejercicio 🔥 correspondiente te pide portar un componente a clase y compararlo línea a línea. La regla del curso no es "usa clases siempre", es "sabe leer y escribir ambas". Acá tocó hooks por mérito propio.

---

## 💻 5. Implementación y código comentado

Trabajamos en `src/features/sales/`. El `db.json` de Fase 3 guarda cada número como `{ raffleId, number, status }` — mismos nombres que el frontend, porque el mock nació ya con la convención de código en inglés. Aun así el thunk **valida la forma antes de confiar en la respuesta**, igual que hacía `fetchRaffles` en Fase 4: un `200 OK` con basura adentro es el fallo más silencioso de los cuatro, y el mock de caos lo inyecta a propósito.

> Endpoints que usa esta fase, **todos definidos en la Fase 3** (`mock/server.js`): `GET /raffles/:id/numbers` para el tablero de una rifa, `POST /raffles/:id/numbers/:number/reserve` y `POST /raffles/:id/numbers/:number/sell`, que responde `409` ante venta duplicada. Son rutas propias montadas sobre json-server, no recursos automáticos, y están en `PROTECTED_ROUTES` — por eso el caos puede inyectarles un `401`.
...
function toReadableError(error) {
  ...
  if (error.response && error.response.status === 409) {
    ...
    return { message: 'Ese número ya fue vendido. Elige otro.', type: 'conflict' };
  }
  if (error.response) {
...
// ── Guarda de forma: convertimos el array del backend al mapa que usa el
// tablero, validando cada elemento. Si la forma no es la esperada, lo
// tratamos como 'malformed' (igual que Fase 4: el 200-con-basura es el
// fallo más silencioso de los cuatro, y el caos lo inyecta a propósito).
function toBoardMap(rawNumbers) {
  ...
  for (const raw of rawNumbers) {
    if (typeof raw.number !== 'string' || typeof raw.status !== 'string') {
      throw { isMalformed: true };
    }
    byNumber[raw.number] = raw.status; // 'available' | 'reserved' | 'sold'
  }
  return byNumber;
}
...
export const reserveNumber = createAsyncThunk(
  ...
  async ({ raffleId, number }, { getState, rejectWithValue }) => {
    ...
    try {
      await apiClient.post(`/raffles/${raffleId}/numbers/${number}/reserve`);
...
export const sellNumber = createAsyncThunk(
  ...
  async ({ raffleId, number, participantId }, { dispatch, getState, rejectWithValue }) => {
    ...
    try {
      await apiClient.post(`/raffles/${raffleId}/numbers/${number}/sell`, { participantId });
...
// Conteos derivados. Recorren el mapa; para 10 000 es barato, pero acuérdate
// de memorizarlos en el componente si los lees en cada render.
...
*Síntoma:* un número aparece `sold` dos veces, o un número vendido "vuelve" a disponible solo. *Causa:* una transición (`numberSoldOptimistic`, `reservationExpired`) que asigna el nuevo estado sin verificar el actual. *Fix mínimo:* la guarda `if (state.byNumber[number] === <expectedStatus>)` antes de cada transición. Es baratísimo y es la diferencia entre una máquina de estados y un `=` suelto.
...
Esta es **la** pieza forense de la Fase 5 (incidente **11** ⭐ de `cuaderno-incidentes.md`): cómo se manifiesta una race condition de venta **en el store**, y cómo cazarla con las herramientas que ya sabes usar desde Fase 4 —Redux DevTools, time-travel, replay—. No reintroducimos DevTools; lo llevamos a su terreno natural.
...
Los ejercicios de "rompe a propósito y observa" (25 y 26) te hacen reproducir exactamente esta secuencia con `CHAOS_LEVEL` alto y dos clicks rápidos, leerla en DevTools, y proponer el blindaje: el rollback no debe revertir a ciegas si entre medio hubo una venta confirmada de otro actor. La solución robusta —cancelar la operación perdedora en el origen— es, de nuevo, territorio de epics (Fase 6). Acá el objetivo es **verla y nombrarla**, que es el 80% de resolver un incidente de concurrencia. Un bug que sabes describir es un bug medio muerto.
...
1. Levanta el mock con `CHAOS_LEVEL=off`, entra al tablero de una rifa en `open` y confirma que se renderizan los 10 000 números en estado `available`.
2. Haz click en un número disponible y verifica en Redux DevTools que se despacha `reserveNumber` y la celda pasa a `reserved` (cambia de color).
3. Reserva un número y véndelo (segundo click). Confirma en el store que quedó `sold` y que su entrada en `reservations` desapareció.
4. Baja `RESERVATION_DURATION_MS` a `5000` (5 s), reserva un número y espera sin tocar nada. Confirma que a los 5 s se despacha `reservationExpired` y vuelve a `available`.
5. Identifica en el código dónde vive el estado de cada número (`byNumber`) y explica en una frase por qué los ids de `setTimeout` **no** están en el store.
6. Cambia el texto "Esta rifa no está abierta a la venta." y fuerza una rifa en `closed` para verlo aparecer.
7. En `NumberCell`, agrega un `data-testid` que incluya el número además del estado (ej. `number-0347-reserved`) y verifícalo en el DOM.
8. Usando `selectStatusCounts`, cuenta cuántos números hay en cada estado y muéstralo en un encabezado sobre el tablero.

**🟡 Intermedio (9–17)**

9. Agrega un componente `NumberLegend` (funcional) que muestre la leyenda de colores disponible/reservado/vendido, leyendo los conteos de `selectStatusCounts`.
10. Agrega un `SalePanel` que muestre el número seleccionado y una cuenta regresiva (mm:ss) hasta que expire su reserva, leyendo `reservations[number].expiresAt`.
11. Haz que al vender un número se cancele explícitamente su timer con `cancelExpiration` y comprueba (con un log temporal) que el `setTimeout` no llega a dispararse.
12. Escribe un JSDoc completo para `sellNumber` documentando el shape de su argumento (`raffleId`, `number`, `participantId`) y qué despacha en éxito y en fallo.
13. Agrega un valor de estado visual `expired` (distinto de `available`) solo en la UI, para que una celda recién expirada parpadee un instante antes de volver a disponible. ¿Va en el store o en el componente? Justifica.
14. Con `CHAOS_LEVEL=high`, fuerza un timeout en `fetchNumbers` y confirma en DevTools que el `error.type` guardado es `'timeout'` y el tablero muestra el mensaje, sin quedar colgado en "Cargando…".
15. Configura el handler de venta de `mock/rafflesNumbersRouter.js` para que devuelva `409` siempre. Vende un número y confirma que el error legible es `type: 'conflict'` con el mensaje "Ese número ya fue vendido…".
16. Refuerza `reserveNumber` para que si el backend devuelve `409` en la reserva (no solo en la venta), también se mapee a `type: 'conflict'`.
17. Agrega un botón "Liberar" en el `SalePanel` que cancele una reserva a mano antes de que expire (despacha `reservationExpired` y `cancelExpiration`). ¿Qué reusas y qué agregas?

**🟠 Difícil (18–24)**

18. Reproduce el bug del timer zombi: comenta el `return () => cancelAllExpirations()` del `useEffect`, reserva un número, navega fuera del tablero, y documenta el `reservationExpired` que aparece en DevTools sin que nadie esté en el tablero. Después restaura el fix.
19. Reproduce el rollback equivocado: modifica `rollbackSale` para que revierta siempre a `'available'` (ignorando `previousStatus`). Vende un número reservado con el handler de venta devolviendo `409` y explica qué inconsistencia queda en el store.
20. Diagnóstico sin correr: te pasan un `reservationExpired` sin la guarda `=== 'reserved'`. Explica con qué secuencia de acciones ese reducer "libera" un número que ya estaba `sold`, y dibuja la secuencia.
21. Optimista al reservar: haz que `reserveNumber` también sea optimista (pinta `reserved` antes de la respuesta del backend) con su propio rollback. ¿Qué nuevo problema de concurrencia introduce respecto a la reserva no optimista?
22. Con el caos en `high`, provoca un `401` en la ruta de venta (token vencido) y confirma por comportamiento que la sesión se cae y te manda a login **sin que `saleSlice` maneje el 401**. Explica qué capa lo hizo.
23. Agrega un contador de "ventas rechazadas por conflicto": cada `sellNumber.rejected` con `type: 'conflict'` incrementa un número. ¿Va en `saleSlice` o en un slice de métricas aparte? Justifica.
24. Time-travel: reserva tres números, vende uno, deja expirar otro. En Redux DevTools viaja a la acción anterior a la expiración y describe qué muestra el tablero. Explica por qué el time-travel puede "resucitar" visualmente una reserva ya expirada.

**🔴 Muy difícil (25–30)**

25. **La race condition, reproducida.** Con `CHAOS_LEVEL=high` y latencia en la ruta de venta, dispara dos ventas casi simultáneas del mismo número reservado (puedes simular dos `dispatch(sellNumber(...))` seguidos). Captura en DevTools la secuencia completa `numberSoldOptimistic → pending → fulfilled → rejected → rollbackSale` y confirma el estado final inconsistente (`sold` en backend, `reserved` en UI).
26. Partiendo del incidente del 25, diseña el blindaje **posible con las herramientas de esta fase** (sin epics): haz que `rollbackSale` no revierta si, entre la mutación optimista y el rechazo, otra venta del mismo número se confirmó. ¿Qué información necesitas guardar para detectarlo? ¿Qué limitación queda que solo un epic resolvería limpio?
27. Escribe una prueba de regresión (Jest 26 + RTL 11, o pseudo-test razonado) que falle si alguien quita la guarda `=== 'reserved'` de `reservationExpired`. ¿Qué secuencia de `dispatch` y qué `expect` la hacen fallar exactamente?
28. Diagnóstico esquivo: un compañero reporta que "a veces, al vender el último número reservado justo cuando expira, queda disponible pero cobrado". Reprodúcelo (pista: la venta y la expiración compiten) y explica el orden de acciones que lo produce. Propón el fix mínimo.
29. Performance: mide con React DevTools Profiler cuántas celdas se re-renderizan al reservar un número **con** y **sin** `React.memo` en `NumberCell`. Documenta la diferencia y explica por qué el `useSelector` puntual por celda es clave.
30. Refactor vs hotfix, escenario PROD: reportan el bug del rollback que pisa una venta legítima (§6). Tienes 30 minutos y PROD comprometido. Escribe el hotfix mínimo y, aparte, describe el refactor "correcto" con epics que harías en Fase 6. Marca cuál es 💸.

**🔥 Opcionales**

- 🔥 Porta `NumberLegend` (o el `SalePanel`) a **class component con `connect()`** y compara línea a línea con la versión de hooks, anotando las equivalencias `useSelector` ↔ `mapStateToProps`. Mantiene viva la práctica de convivencia class/hooks del curso.
- 🔥 Reemplaza el render ingenuo por **virtualización real** con `react-window`: renderiza solo las celdas visibles. Mide el impacto en el tiempo de montaje inicial del tablero y paga la deuda 💸 declarada en la §5.4.
- 🔥 Adelanto a Fase 6: bosqueja (sin integrarlo del todo) cómo se vería el `reservationExpirationEpic` con `timer` + `takeUntil(sellNumber.pending)`. No lo conectes al store todavía; solo escribe el `.pipe()` y explica qué reemplazaría de `reservationTimers.js`.
...
- `A6-redux-clasico-vs-toolkit.md` §7 — thunk manual frente a `createAsyncThunk`, por si te toca leer el estilo viejo en otro módulo del sistema.
- `A7-redux-observable-epica-por-epica.md` — todavía no lo necesitas, pero el ejercicio 🔥 de esta fase te manda ahí: es donde el `setTimeout` de la reserva se convierte en un `timer` con `takeUntil`.
...
- https://redux-toolkit.js.org/api/createSlice — `reducers` (síncronos, como `rollbackSale`) vs `extraReducers` con `builder`. Fíjate en cómo se reusa un case reducer desde `extraReducers` (`reserveNumber.fulfilled` → `numberReserved`).
...
> ⚠️ Las URLs, títulos y secciones pueden haber cambiado o apuntar a versiones más nuevas que las fijadas (RTK 1.8, react-redux 7.2, React 16.14). Verifica siempre que lo que lees aplique a estas versiones. Cuando la doc muestre una API que no existe en ellas, es señal de que estás leyendo material más nuevo de la cuenta.
...
>
> ```bash
> git tag -a fase-05-venta-de-numeros -m "F5 cerrada: <el checklist, en una línea por ítem>"
> ```
>
...
- **El `Map` de timers fuera del store** es una decisión de diseño que Fase 6
  ...
  bucle está cerrado; lo que falta es que *esta* fase anuncie el retiro, para que
  el lector sepa desde el principio que la muleta tiene fecha de vencimiento. →
  Una línea en §5.
- **La guarda de estado de origen en el reducer** —no dejar que
  `reservationExpired` libere un número ya `sold`— es el tipo de validación que
  debería estar en el checklist de hotfix. → Sumarla a la 🪞 retrospectiva del
  cuaderno de incidentes.
- **El optimistic update sin rollback (error común #4)** no tiene ejercicio
  propio, y es el que más caro sale en producción. → Ejercicio 🔴.
...
- **11** ⭐ — *Vendimos el número 0347 dos veces* · Concurrencia · 🔴. La pieza
  central del curso. Sale de la race condition de venta: dos actores, un número,
  un reducer sin guarda de estado de origen. Se ve en Redux DevTools antes que en
  el código, y su fix mínimo (la guarda) es distinto de su fix correcto (la
  cancelación en el origen, que llega en Fase 6).
- **12** — *Un número que ya estaba vendido volvió solo a disponible* ·
  Concurrencia · 🟠. Sale del `rollbackSale` que revierte a ciegas y del timer de
  reserva que sobrevive al desmontaje. Dos causas que producen el mismo síntoma:
  distinguirlas es el ejercicio.
