...
import apiLottery from '../../../api/apiLottery'; // instancia axios nueva, ver 5.8
import { logout } from '../../auth/authSlice';       // el type sale del creator, nunca a mano
...
Exporta los nuevos creators junto a los de Fase 4:
...
// Intervalo del polling de resultados. Configurable: súbelo para bajar
// la presión sobre la lotería, bájalo para ver el resultado antes.
...
### 5.8 La instancia HTTP de la lotería — `src/api/apiLottery.js`
  ...
  action$.pipe(
    ...
    switchMap(({ payload }) =>
      timer(0, 5000).pipe(
        ...
        takeUntil(
          action$.pipe(ofType('STOP_BOARD_REFRESH', logout.type))
        )
      )
    )
  );
...
> Con esto la deuda de Fase 6 queda cerrada: el refresco del tablero ya no es un leak. Si lo montas en el `rootEpic`, es otra línea en el `combineEpics`, igual que `pollingEpic`.
...
**🟢 Fácil (1–7)**
1. Escribe una prueba unitaria de `isPastClosing` con un `now` fijo antes y después de `closesAt`, incluyendo el caso exacto de la igualdad (`now === closesAt`). ¿Cuál es el comportamiento en el borde y por qué elegimos `>=`?
2. Verifica con `node` que `new Date("2026-08-30T22:00:00-05:00").getTime()` da el mismo número corriendo el proceso con `TZ=America/Bogota` y con `TZ=Europe/Madrid`. Explica por qué.
3. Pásale a `isPastClosing` un `closesAt` corrupto (`"no-soy-fecha"`) y confirma que devuelve `true` (fail-safe). Justifica por qué "cerrado" es el default seguro y no "abierto".
4. En `NumberCell`, muestra un 🔒 junto a los números disponibles cuando la rifa cerró por hora dura. Confirma que el botón queda `disabled`.
5. Agrega `POLLING_INTERVAL_MS` como constante configurable y cámbialo a 1000ms; observa en Network que el ritmo de `GET /results` se acelera.
6. Levanta el mock de lotería de Fase 7 y confirma con `curl` que `GET /results/1` devuelve `204` antes del cierre y `200` con `winningNumber` después (puedes adelantar `CLOSES_AT[1]` a una hora pasada para probar).
7. Escribe `msUntilClosing` en un componente que muestre "Cierra en Xs" y confirma que llega a cero en el instante correcto.

**🟡 Intermedio (8–16)**
8. Compón la guarda completa de cierre en `NumberCell`: `selectIsRaffleClosedByStatus(id)(state) || isPastClosing(raffle.closesAt)`. Explica por qué hacen falta las dos condiciones y no una sola.
9. Agrega el rechazo por `type: 'closed'` en el thunk `sellNumber` y muestra en la UI el mensaje en español cuando alguien intenta vender pasado el cierre (por ejemplo, con un click viejo en vuelo).
10. Despacha `START_POLLING` al montar el tablero y `STOP_POLLING` al desmontarlo (con `useEffect` cleanup). Confirma en Network que el polling arranca y para con el ciclo de vida del componente.
11. Provoca un `500` en `/results` con el caos `high` y verifica en Network los gaps del backoff (500ms, 1s, 2s) antes de que el tick emita `pollingFailed`.
12. Explica por qué el `catchError` del `pollingEpic` está dentro del `mergeMap` del tick y no envolviendo el `timer`. Muévelo afuera a propósito y describe qué se rompe.
13. Agrega `raffleClosed` disparado por un epic o efecto cuando `isPastClosing` se vuelve verdadero, y confirma que la transición `open → closed` ocurre una sola vez (guarda de estado de origen).
14. Confirma que `resultReceived` solo transiciona `closed → resolved` y que un resultado que llega con la rifa aún en `open` se ignora. Fuerza el caso enviando la acción a mano desde Redux DevTools.
15. Distingue en Network un `204` de un `500`: muestra que el `204` no genera `pollingFailed` y el `500` sí (tras agotar reintentos).
16. Usa `selectRaffleResult(raffleId)` para mostrar el número ganador en la UI cuando la rifa pasa a `resolved`.

**🟠 Difícil (17–24)**
17. **Diagnóstico:** te dan un `pollingEpic` con `catchError` afuera del tick. Reproduce el síntoma (el polling muere al primer `500`), localiza la línea y aplica el fix mínimo.
18. **Diagnóstico:** un `pollingEpic` tiene el `takeUntil` sin `resultReceived.type`. Muestra en Network que sigue poleando tras recibir el ganador y corrígelo. Escribe el post-mortem.
19. **Diagnóstico:** la guarda de cierre usa `new Date(closesAt).getHours() >= 22` en vez de comparar instantes. Reproduce el bug de medianoche cambiando la TZ del sistema y corrige a comparación de `getTime()`.
20. Consolida el `boardRefreshEpic` corregido (5.10) en el `rootEpic` y confirma con Network que su `interval` ya no se escapa al logout. Compara el diff con la versión rota de Fase 6.
21. Reproduce un tick que llega **tarde** tras `STOP_POLLING` (sube la latencia del caos y despacha `STOP_POLLING` justo tras un tick). Muestra que el `filter` + `takeUntil` evitan que emita, y discute cuándo harías falta el tipo de error `'stale'`.
22. Mide (Performance de DevTools) cuántas peticiones a `/results` genera el polling en 30s con `POLLING_INTERVAL_MS` de 3000 vs 1000, y razona el trade-off presión-de-red / latencia-al-resultado.
23. Escribe una prueba de integración (a mano, sin marble aún) que despache `START_POLLING`, simule un `200` con ganador y verifique que llegan en orden `resultReceived` y que el polling se detiene.
24. **Refactor:** colapsa las tres guardas de cierre (UI, thunk, epic) a un único selector derivado `selectIsRaffleClosed(raffleId)` que combine status + hora dura, resolviendo dónde vive el `new Date()` sin romper la memoización. Justifica la ubicación.

**🔴 Muy difícil (25–30)**
25. **Diagnóstico + regresión:** repo con bug intermitente — a veces, tras cerrar la rifa, un último `GET /results` con un resultado válido llega y pinta `resolved`, pero *otras veces* un tick tardío tras `STOP_POLLING` intenta re-emitir. Encuentra la carrera entre el `takeUntil` y la petición en vuelo, propón el fix y la prueba de regresión.
...
**🔥 Opcionales**
- 🔥 Reemplaza `Date` nativo por `date-fns` (o `luxon`) **solo** para *formatear* el contador "Cierra en 2h 15m" en la zona del usuario, dejando la comparación de instantes en `Date`. Justifica por qué la librería suma en formateo pero no en la guarda.
- 🔥 Agrega un `POST /_chaos` al mock de lotería para cambiar el `CHAOS_LOTTERY_LEVEL` en caliente (la deuda 💸 que Fase 3 dejó anotada) y úsalo para provocar rachas de `500` durante el polling sin reiniciar el proceso.
- 🔥 Materializa el tipo de error `'stale'`: cuando un resultado llega tras `STOP_POLLING`, en vez de descartarlo en silencio, deja rastro con `type: 'stale'` para trazabilidad, y decide si la UI debe mostrarlo.
...
- `A7-redux-observable-epica-por-epica.md` — la tabla maestra de operadores y el patrón de `takeUntil` + `timer` que este `pollingEpic` aplica. Si dudas entre `switchMap` y `mergeMap`, la respuesta está ahí.
- `A11-marble-testing.md` §8 — cómo se prueba que este polling **para**.
...
- https://rxjs.dev/api/operators/switchMap y https://rxjs.dev/api/operators/mergeMap — reléelas para decidir el nivel externo vs interno del polling.
...
**Video / apoyo**
- Busca en YouTube "RxJS polling with timer takeUntil" y "RxJS exponential backoff retryWhen". Prefiere contenido 2020–2022 para que las versiones de RxJS coincidan con las del curso.
...
> ⚠️ URLs, títulos y contenidos pueden estar desactualizados; verifícalos antes de compartir. La doc de RxJS enlazada es la de **6.x**: confirma que no estás leyendo la de RxJS 7, que deprecó `retryWhen` y cambia algunas firmas. La página de `Date` de MDN es estable, pero el comportamiento depende de la implementación del motor: prueba siempre con `TZ` distintos.
...
>
> ```bash
> git tag -a fase-07-cierre-polling-resultado -m "F7 cerrada: <el checklist, en una línea por ítem>"
> ```
>
...
- **`serverNow` 💸 no tiene fase de pago.** Se difiere desde acá a "Fase 8 /
  forense", en Fase 8 se difiere de nuevo, y en Fase 11 sigue abierto. Un
  pendiente que se difiere tres veces ya no es un pendiente: es una decisión. →
  Declararlo deuda permanente documentada, o darle un ejercicio 🔥 con fecha.
...
- **El tipo `'stale'` está reservado en el catálogo de `error.type` y no se
  ...
  futuro.
- **La doble fuente de verdad del cierre 💸** se paga parcialmente con los
  selectores derivados de §5. → Confirmar que quede **un solo** lugar que
  responda "¿está cerrada?", y decirlo explícitamente en el cierre.
...
- **16** — *La rifa siguió vendiendo después de la hora de cierre* · Tiempo · 🟠.
  Sale del error común #1: comparar componentes de fecha en vez de instantes. El
  síntoma cambia según la zona horaria del navegador, así que solo se reproduce
  si el enunciado dice en qué TZ estaba el cliente.
- **17** — *El resultado nunca llega y no aparece ningún error* · RxJS / epics · 🟠.
  Sale del `catchError` colocado fuera del `timer`: el primer 500 de la lotería
  mata el polling en silencio. Convive con una segunda causa plausible —tratar el
  `204` como error— y distinguirlas es el ejercicio.
